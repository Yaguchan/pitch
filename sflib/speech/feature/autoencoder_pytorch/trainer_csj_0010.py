...
from ....sound.sigproc.noise import NoiseAdder, IntermittentNoiseAdder, MutipleNoiseAdder
...
class SpectrogramImageAutoEncoderTrainerCSJ0010(
    ...
    def get_optimizer(self):
        return optim.Adam(self.autoencoder.parameters())
...
def train(autoencoder_version, device=None):
    from .base import construct_autoencoder
    autoencoder = construct_autoencoder(autoencoder_version)
    trainer = SpectrogramImageAutoEncoderTrainerCSJ0010(device=device)
    trainer.set_autoencoder(autoencoder)
    early_stopper = EarlyStopper(patience=3, verbose=True)
    trainer.train(additional_callbacks=[early_stopper], epoch=40)
