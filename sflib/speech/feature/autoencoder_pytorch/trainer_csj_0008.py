...
class SpectrogramImageAutoEncoderTrainerCSJ0008(
    ...
    def build_vaid_dataset_csj(self,
        ...
        from ....corpus.speech.csj import CSJ
        csj = CSJ()
        id_list = csj.get_id_list(cond='[AS].*1$',
                                  # cond='A01M0015',
                                  )
        sidm = csj.get_spec_image_data_manager(generator, noise_adder)
        return SpectrogramImageDataset(sidm, id_list, max_utterance_num)
...
def train(autoencoder_version, device=None):
    ...
    trainer = SpectrogramImageAutoEncoderTrainerCSJ0008(device=device)
    trainer.set_autoencoder(autoencoder)
    early_stopper = EarlyStopper(patience=3, verbose=True)
    trainer.train(additional_callbacks=[early_stopper])
