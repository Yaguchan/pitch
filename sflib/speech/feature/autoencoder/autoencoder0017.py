...
class SpectrogramImageAutoEncoder0017(SpectrogramImageAutoEncoder):
    def __init__(self):
        super().__init__()
        ...
        x = BatchNormalization()(x)
        # CNN-5
        x = Conv2D(128, (7, 1), activation='relu', padding='same', strides=(3, 1),
                   kernel_initializer='he_normal', bias_initializer='zeros')(x)
        x = BatchNormalization()(x)
        # CNN-6
        x = Conv2D(256, (4, 1), activation='relu', padding='valid', strides=(1, 1),
                   kernel_initializer='he_normal', bias_initializer='zeros')(x)
        x = BatchNormalization()(x)
        # Reshape
        x = Reshape((256,))(x)
        ...
        x = Reshape((1, 1, 256))(x)
        # Inverse CNN-6
        x = Conv2DTranspose(128, (4, 1), activation='relu', padding='valid',
                            strides=(1, 1),
                            kernel_initializer='he_normal', bias_initializer='zeros')(x)
        x = BatchNormalization()(x)
        # Inverse CNN-5
        x = Conv2DTranspose(64, (7, 1), activation='relu', padding='same',
                            strides=(3, 1),
                            kernel_initializer='he_normal', bias_initializer='zeros')(x)
