...
# 0001との違い，出力（ボトルネック特徴）にBatchNormalizationをかけるのをやめる．
...
class SpectrogramImageAutoEncoder0002(SpectrogramImageAutoEncoder):
    def __init__(self):
        super().__init__()
        ...
        x = BatchNormalization()(x)
        # CNN-3
        x = Conv2D(64, (7, 5), activation='relu', padding='same', strides=(3, 2),
                   kernel_initializer='he_normal', bias_initializer='zeros')(x)
        x = BatchNormalization()(x)
        # CNN-4
        x = Conv2D(128, (7, 5), activation='relu', padding='same', strides=(3, 2),
                   kernel_initializer='he_normal', bias_initializer='zeros')(x)
        x = BatchNormalization()(x)
        # FC-1
        x = Flatten()(x)
        ...
        x = Conv2DTranspose(64, (7, 5), activation='relu', padding='same',
                            strides=(3, 2),
                            kernel_initializer='he_normal', bias_initializer='zeros')(x)
        x = BatchNormalization()(x)
        # Inverse CNN-3
        ...
        x = BatchNormalization()(x)
        # Inverse CNN-1
        x = Conv2DTranspose(1, (5, 5), activation='relu', padding='same',
                            strides=(2, 2),
                            kernel_initializer='he_normal', bias_initializer='zeros')(x)
