# coding: utf-8
from sflib.sound.sigproc.spec_image_torch \
...
class SpecImageDataTorch:
    def __init__(self,
        ...
        if num_samples <= 0:
            ...
            self.__num_images = int(
                np.ceil(num_samples / generator.num_samples_per_image_shift)) + 1
    ...
    def get_image(self, noised=False):
        # print("{} get_image noised={}".format(self.wav_data, noised))
        
        if noised and self._noise_adder is None:
            raise ValueError()
        # print("BEFORE GET SAMPLES")
        wav = self._wav_data.wav
        # print("AFTER  GET SAMPLES")
        # 必要に応じてゼロ詰めをする
        num_samples = len(wav)
        ...
        if noised and self._noise_adder is not None:
            ...
            self.__cache_noised = self._generator.input_wave(wav)
            # return self.__cache_noised
            result = self.__cache_noised
        else:
            self._generator.reset()
            self.__cache_clean = self._generator.input_wave(wav)
            # return self.__cache_clean
            result = self.__cache_clean
        # print("DONE")
        return result
    ...
    def __repr__(self):
        return "SpecImageDataTorch(cached=%s, num_images=%d, trans=%s)" % (
            self.__cache_clean is not None, self.__num_images,
            self._wav_data.trans.trans)


class SpecImageDataManagerTorch:
    def __init__(self,
    ...
    def get(self, id):
        ...
        for wav_data_list in wav_datas:
            ...
            for wav_data in wav_data_list:
                data_list.append(
                    SpecImageDataTorch(wav_data, self.__generator,
                                       self.__noise_adder))
...
class SpecImageRandomAccessorTorch:
    def __init__(self,
                 manager: SpecImageDataManagerTorch,
                 id_list,
    ...
    def __build_data_list(self):
        ...
        # IDごとにまとめたwav_dataのリストのリスト．
        # シャッフルする際は，このデータをシャッフルする．
        # 余計なI/Oを避けるため
        count = 0
        wav_data_for_id_list = []
        for id in tqdm.tqdm(self.__id_list):
            wav_datas = self.__manager.get(id)
            wav_data_list_to_add = []
            for wav_data_list in wav_datas:
                if self.__max_num_data_for_id is not None and \
                   len(wav_data_list) > self.__max_num_data_for_id:
                    wav_data_list = copy.copy(wav_data_list)
                    ...
                    wav_data_list = np.array(wav_data_list)[ind]
                    wav_data_list = \
                        wav_data_list[:self.__max_num_data_for_id].tolist()
                wav_data_list_to_add.extend(wav_data_list)
            wav_data_for_id_list.append(wav_data_list_to_add)
                    
        data_list = []
        base_indices = [0]
        for wav_list in wav_data_for_id_list:
            for wav_data in wav_list:
                data_list.append(wav_data)
                base_indices.append(base_indices[-1] + wav_data.num_images)

        self.__wav_data_for_id_list = wav_data_for_id_list
    ...
    def shuffle(self):
        ...
        np.random.shuffle(self.__wav_data_for_id_list)
        data_list = []
        base_indices = [0]
        for wav_list in self.__wav_data_for_id_list:
            for wav_data in wav_list:
                data_list.append(wav_data)
                base_indices.append(base_indices[-1] + wav_data.num_images)
        self.__spec_image_data_list = data_list
