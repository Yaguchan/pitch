...
class TurnUtteranceVadCrossEntropyLoss(nn.Module):
    def __init__(self,
        ...
        self._alpha_loss = nn.MSELoss(reduction='sum')
        ...
        self._ones = nn.Parameter(torch.ones(1, 1))
        self._count = 0
    ...
    def forward(self, y, t, y_len):
        ...
        y_dt = torch.nn.functional.hardtanh(y_dt)
        
        # y_alpha = torch.exp(-y_alpha)
        # y_alpha = torch.relu(y_alpha + 0.4) + 0.1
        # y_alpha = torch.exp(y_alpha) + 0.1
        # y_alpha = torch.nn.functional.softplus(y_alpha) + 0.1e-6
        y_alpha = torch.sigmoid(y_alpha)
        # y_alpha = torch.sigmoid(y_alpha - 10.0)
        # y_alpha = y_alpha ** 2
        # y_alpha = y_alpha.abs()
        # y_alpha = torch.nn.functional.softplus(y_alpha)
        
        # t_dt = torch.tanh(t_dt)
        # t_dt = torch.tanh(y_alpha * t_dt)
        # coeff = -0.9 * y_alpha * (y_dt ** 2) + 1.0
        # a_pre = 0.5 * (torch.nn.functional.hardtanh(y_alpha) + 1.0)
        a_pre = y_alpha
        coeff = -0.9 * a_pre * (y_dt ** 2) + 1.0
        # t_dt_orig = t_dt
        t_dt = torch.tanh(coeff * t_dt)

        # y_dt_orig = 0.5 * torch.log((1+y_dt) / (1-y_dt)) / coeff
        
        # if self._count % 5 == 0:
        #     dd = torch.stack([y_dt, t_dt, coeff, y_alpha, y_dt_orig, t_dt_orig], dim=2)
        #     print(dd[:10, 0])
        # self._count += 1
        ...
        for i in range(batch):
            ...
            # total_loss += self._weight_alpha * self._alpha_loss(
            #     y_alpha[:y_len[i], i],
            #     torch.repeat_interleave(self._ones, y_len[i], dim=0))
            # total_loss += self._weight_z * torch.sum(torch.exp(-y_dt[:y_len[i], i]**2))
...
class TurnDetectorTrainer0027(TurnDetectorTrainer):
