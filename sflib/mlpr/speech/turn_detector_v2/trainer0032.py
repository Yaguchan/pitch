# 発話終了までの予測時間が正の場合，係数5
# 発話終了までの予測時間が負の場合，係数1
from .base import \
    TorchTrainerForTurnDetector, TurnDetectorTrainer, TurnDetector
...
class TurnUtteranceVadCrossEntropyLoss(nn.Module):
    ...
    def forward(self, y, t, y_len):
        ...
        t_dt[t_dt > 0] = torch.tanh(5.0 * t_dt[t_dt > 0])
        
        y_vad = torch.sigmoid(y_vad)

        # if self._count % 5 == 0:
        #     dd = torch.stack([y_dt, t_dt, y_vad, t_vad], dim=2)
        #     print(dd[:10, 0])
        #     # print(t_dt[:10, 0])
        # self._count += 1
...
class TurnDetectorTrainer0032(TurnDetectorTrainer):
    def __init__(self, backprop_len=500):
        super().__init__()
