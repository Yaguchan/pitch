# 21
#  2X系統は，TURN終了を0として，前後のズレを推定するモデル．
#    ターゲットは，TURN終了を0，
#          TURN以前の位置に終了までの時刻[s]（正になる）
#          TURN以後（次の発話開始まで），直前のTURN終了までの時間[s]（負になる）
#    補助情報はVAD
#    モデルの出力は，
#       ・ターゲットまでの予測時刻相当
#       ・補助傾き情報（なるべく1に近づけるが，問題が難しい場合は大きくなる）
#       ・VADのON, OFF（今回は1次元で BCELoss で最適化する）
...
class TurnDetectorTrainer0021(TurnDetectorTrainer):
