# 22
#  21基本で，exp(alpha)^2 を最小化する(0に近づける)という損失を入れる
...
class TurnUtteranceVadCrossEntropyLoss(nn.Module):
    def __init__(self,
        ...
        self._zeros = nn.Parameter(torch.zeros(1, 1))
    ...
    def forward(self, y, t, y_len):
        ...
        y_alpha = torch.exp(y_alpha) + 1e-6
        y_dt = torch.tanh(y_dt) / y_alpha
        t_dt = torch.tanh(t_dt) / y_alpha
        ...
        for i in range(batch):
            ...
            total_loss += self._weight_alpha * self._alpha_loss(
                y_alpha[:y_len[i], i],
                torch.repeat_interleave(self._zeros, y_len[i], dim=0))
...
class TurnDetectorTrainer0022(TurnDetectorTrainer):
