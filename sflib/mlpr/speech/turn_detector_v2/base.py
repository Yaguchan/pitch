...
class TurnDetectorFeatureExtractor(metaclass=ABCMeta):
    ...
    def __init__(self):
        # class name check
        m = re.match(r'TurnDetectorFeatureExtractor(\d+)',
                     self.__class__.__name__)
        if m is None:
            raise RuntimeError('class name should match with' +
                               'r"TurnDetectorFeatureExtractor\\d+"')
...
class TurnDetector(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        """
        # class name check
        m = re.match(r'TurnDetector(\d+)', self.__class__.__name__)
        if m is None:
            raise RuntimeError('class name should match with' +
                               'r"TurnDetector\\d+"')
        self.__number = int(m[1])
    ...
    @property
    def filename_base(self):
        return 'TD{:02d}T{:02d}{}'.format(self.__number, self.__trainer_number,
    ...
    # ---- autoencoder_v2, phone_type_writer_v2にほぼ同様の記述があるため
    # ---- mixin などで対処したいところ
...
class _FeatureExtractorWrapper:
    ...
    def __init__(self, feature_extractor: TurnDetectorFeatureExtractor,
                 wav_list: list):
...
class TorchTrainerForTurnDetector(TorchTrainer):
    ...
    def _forward(self, batch, update=True):
        ...
        feature_extractor_wrapper = _FeatureExtractorWrapper(
            self._turn_detector.feature_extractor, wavs)
        ...
        while not feature_extractor_wrapper.finished:
            ...
            # print(lengths)
            # import ipdb; ipdb.set_trace()
            # feat_padded, _ = pad_packed_sequence(feat_packed)
            # y_padded = self._turn_detector.forward_core_padded(feat_padded)
            y_packed = self._turn_detector.forward_core(feat_packed)
            ...
            # デタッチする
            self._turn_detector.feature_extractor.detach()
...
class TurnDetectorTrainer(metaclass=ABCMeta):
    ...
    def train(self, turn_detector: TurnDetector):
...
def construct_feature_extractor(feature_extractor_number,
    ...
    """
    module_name = "sflib.mlpr.speech.turn_detector_v2." + \
                  "feature_extractor{:04d}".format(feature_extractor_number)
    class_name = "TurnDetectorFeatureExtractor" + \
...
def construct_turn_detector(turn_detector_number,
    ...
    module_name = "sflib.mlpr.speech.turn_detector_v2." + \
        "turn_detector{:04d}".format(turn_detector_number)
    class_name = "TurnDetector{:04d}".format(turn_detector_number)
    import importlib
    mod = importlib.import_module(module_name)
    cls = getattr(mod, class_name)
...
def construct_trainer(trainer_number, *args, **kwargs):
    ...
    class_name = "TurnDetectorTrainer{:04d}".format(trainer_number)
    import importlib
    mod = importlib.import_module(module_name)
    cls = getattr(mod, class_name)
