...
class TurnUtteranceVadCrossEntropyLoss(nn.Module):
    ...
    def forward(self, y, t, y_len):
        ...
        y_alpha = torch.sigmoid(y_alpha) * 0.9 + 0.1

        t_dt_orig = t_dt.clone()
        t_dt[t_dt < 0] = torch.tanh(t_dt[t_dt < 0])
        t_dt[t_dt > 0] = torch.tanh(y_alpha[t_dt > 0] * t_dt[t_dt > 0])
        
        y_vad = torch.sigmoid(y_vad)

        # if self._count % 5 == 0:
        #     dd = torch.stack([y_dt, t_dt, t_dt_orig, y_alpha, y_vad, t_vad], dim=2)
        #     print(dd[:10, 0])
        #     # print(t_dt[:10, 0])
        # self._count += 1
...
class TurnDetectorTrainer0028(TurnDetectorTrainer):
