# 0002 がベース．学習データの絞り込み方を完全ランダムではなくした．
...
class PhoneTypeWriterTrainerCSJRWCP0005(PhoneTypeWriterTrainer):
    ...
    def _build(self):
        ...
        siseq_rwcp_train = SpecImageTransSequence(
            100, sim_rwcp, id_list_rwcp_train,
            # self.phone_type_writer.feature_extractor, 100)
            self.phone_type_writer.feature_extractor)
        # 学習用のシーケンス
...
def train_with_autoencoder(phone_type_writer_version,
    ...
    trainer = PhoneTypeWriterTrainerCSJRWCP0005(phone_type_writer)
    trainer.train()
