...
class PhoneTypeWriter0005(PhoneTypeWriter):
    ...
    def _build(self):
        ...
        inner = TimeDistributed(Dense(128, activation='tanh',
                                      name='dense2'))(inner)
        inner = LSTM(
            128, return_sequences=True, activation='tanh', name='lstm1')(inner)
        inner = TimeDistributed(Dense(64, name='dense3', activation='tanh'))(inner)
        middle_out = inner
        ...
        middle_out_func = K.function([input_data, input_length], [middle_out])
        ...
        self._model = model
        self._middle_out = middle_out_func
    ...
    def calc_middle_out(self, x):
        x = self.feature_extractor.calc(x)
        x = x.reshape((1, ) + x.shape)
        middle_out = self._middle_out([x])
        return middle_out[0]
