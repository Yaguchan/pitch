...
class CollateDurationInfo:
    ...
    def _collate(self, duration_info_list, ch):
        ...
        # s = 16000
        # e = 16000 + 5 * 16000
        # wav_list = [wav[s:e] for wav in wav_list]
        ...
        # s = 50 * 60
        # e = 50 * 60 + 5 * 50 * 60
        # labels_list = [l[s:e] for l in labels_list]
...
class TimingLoss(nn.Module):
    ...
    def forward(self, y, target, y_len):
        ...
        # print('z(t): {:.5f}({}), nz(t): {:.5f}({})'.format(
        #     z_loss.item()/z_count.item(), z_count.item(),
        #     nz_loss.item()/nz_count.item(), nz_count.item()))

        loss_count = z_count + nz_count
        if loss_count > 0:
            loss = (z_loss + nz_loss) / loss_count
        else:
            loss = 0
...
class TurnDetector1dTrainer0099(TurnDetector1dTrainer):
    def __init__(self, backprop_len=30000):
        super().__init__()
