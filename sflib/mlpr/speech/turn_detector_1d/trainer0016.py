# 0015 -> 0016
#   不正解タイミングのロスを再度有効化．
#   ただし，|y - 0.7| とする（微分が1になるように）これで正解と同等に下げにいくはず．
#   重みは小さいけど...
# 0013 -> 0015
#   不正解タイミングのロスを無しにし，
#   正解タイミングのロスを厳しくした．
#   厳密に0.8に近くように， < 0.8 の場合は -0.8 log(y/0.8)， > 0.8 の場合は - 0.2 log((1-y)/0.2)
# 0003 -> 0013
...
class TimingLoss(nn.Module):
    ...
    def forward(self, y, target, y_len):
        ...
        if z_count > 0:
            ...
            yt_sub = (yt_sub + 0.0001) / 1.0002
            yt_sub_over = yt_sub[yt_sub > 0.8]
            loss_over = -0.2 * torch.log((1.0 - yt_sub_over) / 0.2)
            yt_sub_under = yt_sub[yt_sub < 0.8]
            loss_under = -0.8 * torch.log(yt_sub_under / 0.8)
            # print("over")
            # print(loss_over)
            # print("under")
            # print(loss_under)
            z_loss = loss_over.sum() + loss_under.sum()
        else:
            z_loss = 0
        ...
        if nz_count > 0:
            ...
            target = torch.ones_like(yt_sub, device=self._device) * 0.7
            nz_loss = F.l1_loss(yt_sub, target, reduction='sum')
        else:
            nz_loss = 0
        # nz_count = 0
        # nz_loss = 0.0
...
class TurnDetector1dTrainer0016(TurnDetector1dTrainer):
