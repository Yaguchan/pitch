# 0003 -> 0004
#   1つのepoch中では，ランダムに1分間のデータを切り出して学習し，
#   epoch数を大きくした(100)
# 0002 -> 0003
...
class CollateDurationInfo:
    ...
    def _collate(self, duration_info_list, ch):
        ...
        for i in range(len(wav_list)):
            length_in_secoonds = len(wav_list[i]) / 16000.0 - 61.0
            start_in_seconds = float(np.random.rand(1) * length_in_secoonds)
            
            s = int(start_in_seconds * 16000)
            e = s + 16000 * 60
            wav_list[i] = wav_list[i][s:e]

            s = int(start_in_seconds * self._frame_rate)
            e = s + 50 * 60
            labels_list[i] = labels_list[i][s:e]
...
class TurnDetector1dTrainer0004(TurnDetector1dTrainer):
