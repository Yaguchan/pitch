# 0014 -> 0017
#   y(t)が0.8を超えたところを（もちろんf(t)==1のところだけ），
#   一率にで0.799になるように下げる．
#   均衡を取れるようにするためには結局これが一番良いような気がする．
# 0013 -> 0014
...
class TimingLoss(nn.Module):
    ...
    def forward(self, y, target, y_len):
        ...
        if nz_count > 0:
            ...
            target = torch.ones_like(yt_sub, device=self._device) * 0.799
...
class TurnDetector1dTrainer0017(TurnDetector1dTrainer):
