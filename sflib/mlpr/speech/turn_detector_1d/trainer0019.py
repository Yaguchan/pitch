# 0018 -> 0019
#   タイミングは0018と同じだが，その際の目標を 0.2 （0.8の反対）とする．0014の成績が依然良いため．
# 0013 -> 0018
#   下げる学習をするタイミングを，y(t) が 0.8 を超えたタイミングではなく，
#   y(t)が強制的に0になる直前に0.8を超えていた場合（つまり，y(t) >= 0.8 & y(t + 1) < 0.01)
#   の場合に0.799まで落とすようにする．
# 0003 -> 0013
...
class TimingLoss(nn.Module):
    ...
    def forward(self, y, target, y_len):
        ...
        yt1_ = torch.cat([yt[1:, :, :], yt[-1:, :, :]], dim=0)
        nz_flag = (zt < 0.1) & (ft > 0.9) & (yt >= 0.8) & (yt1_ < 0.1)
...
class TurnDetector1dTrainer0019(TurnDetector1dTrainer):
