...
class TurnDetectorModel(TurnDetectorTorchModel):
    def __init__(self, in_feature_dim):
        ...
        self.linear_out_turn = nn.Linear(64, 3)  # 0, 1(TURN), 2(SHORT)
        self.linear_out_utterance = nn.Linear(64, 2)  # 0, 1(ON)
    ...
    def forward_padded(self, feat: torch.Tensor, lengths: torch.Tensor=None) -> torch.Tensor:
        ...
        h_linear1 = torch.tanh(self.linear_1(h_lstm1_flat))
        h_turn = self.linear_out_turn(h_linear1)
        h_utterance = self.linear_out_utterance(h_linear1)
        h_vad = self.linear_out_vad(h_linear1)
        h_all = torch.cat([h_turn, h_utterance, h_vad], dim=1)
...
class TurnDetector0001(TurnDetector):
