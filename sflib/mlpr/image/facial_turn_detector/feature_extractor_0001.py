...
class InputImageExtractor:
    ...
    @staticmethod
    def detect_face_in_rect(image, rect, detector, detect_width=150):
        """指定された枠内で顔を見つける．
        位置の情報は元の座標に戻される．
        """
        ((x1, y1), (x2, y2)) = rect
        subimage = image[y1:y2, x1:x2]
        if x2 - x1 > detect_width:
            ratio = detect_width / (x2 - x1)
            detect_height = int(np.ceil((y2 - y1) * ratio))
            subimage = cv2.resize(subimage, (detect_width, detect_height))
        else:
            ratio = 1.0
        ...
        rx1 = int(rx1 / ratio + x1)
        rx2 = int(np.ceil(rx2 / ratio + x1))
        ry1 = int(ry1 / ratio + y1)
        ry2 = int(np.ceil(ry2 / ratio + y1))
...
class FacialTurnDetectorFeatureExtractor0001(FacialTurnDetectorFeatureExtractor
                                             ):
    def __init__(self,
                 encoder_version,
                 n_person,
    ...
    def feature_dim(self):
        """特徴ベクトルの次元数
        """
        return np.sum(self._encoder.feature_dims)
    ...
    def calc(self, img_seq_list: list) -> list:
        ...
        for img_seq in img_seq_list:
            ...
            for image in img_seq:
                in_image = self._input_image_extractor.extract(image)
                # print(in_image)
                if in_image is None:
            ...
            feat_list.append(torch.cat(feat_seq, dim=0))
        return feat_list
