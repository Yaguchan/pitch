...
class InputImageExtractor:
    ...
    def extract(self, image: np.array):
        ...
        if face is None:
            self._prev_face = None
            return None, np.array([0.0] * (68 * 2 + 3), np.float32)
...
class FacialTurnDetectorFeatureExtractor0007(FacialTurnDetectorFeatureExtractor
                                             ):
    def __init__(self,
                 encoder_version,
                 n_person,
    ...
    def feature_dim(self):
        """特徴ベクトルの次元数
        """
        return np.sum(self._encoder.feature_dims[2:]) + 68 * 2 + 3
