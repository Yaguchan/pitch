# lfw_0002をベースに，Rinusのデータ100人分を付け加えて学習する
import numpy as np
import torch
...
from .....corpus.video.rinus.process import PPESv2Data as Rinus_PPESv2Data
...
class PPESv2EncoderTrainerRinusLFW0001(PPESv2EncoderTrainer):
    ...
    def build_torch_trainer(self, ppes_encoder: PPESv2Encoder):
        ...
        # Rinus
        rinus_ppes_data = Rinus_PPESv2Data()
        ...
        id2index = {'unknown': 0}
        count = 1
        for i in tqdm.tqdm(range(len(rinus_ppes_data))):
            ppes = rinus_ppes_data[i]
        ...
        dataset_train_rinus = PPESv2Dataset(rinus_ppes_data)
        dataset_train_lfw = PPESv2Dataset(lfw_ppes_data, idx_train)
        dataset_train = ConcatDataset([dataset_train_rinus, dataset_train_lfw])
...
def construct0001(device=None):
    ...
    trainer = PPESv2EncoderTrainerRinusLFW0001(ppes_encoder)
    return trainer
