...
alignment_data_path = path.join(
    config.get_package_data_dir(__package__), 'data', 'alignment_data.npz')
...
def make_alignment_data():
    ...
    if not path.exists(path.dirname(alignment_data_path)):
        os.makedirs(
            path.dirname(alignment_data_path),
            mode=0o755,
            exist_ok=True)
    ...
    def align(self, image, landmarks):
        ...
        for i, triangle in enumerate(self.triangles):
            ...
            warpedImage = cv2.warpAffine(
                image[minPosition[1]:(maxPosition[1] + 1), minPosition[0]:(
                    maxPosition[0] + 1)], affineTransform,
                (self.dim, self.dim))
